    ...
    if(storeData.offers.length > 0) {
      ...
      var OfferspopupContent = `
        <div class='popup'>
          ...
          ${storeData.distance < 500 ? '<input type="submit" value = "Αξιολόγηση" onclick="review();" " id="popupButton"></input>':"showPage(reviews)"}
          `).join('')}
          ${storeData.distance < 500 ? '<button id="popupButton">Υποβολή προσφοράς</button>':""}
        </div>`;
